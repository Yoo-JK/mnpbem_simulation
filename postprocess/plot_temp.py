...
def _to_real_array(arr):
    """
    Convert any array to real float, handling HDF5 structured complex dtypes.

    HDF5 (MATLAB v7.3) stores complex numbers as structured dtype:
        dtype([('real', '<f8'), ('imag', '<f8')])
    which np.iscomplexobj() does NOT detect. This function handles both
    native numpy complex and HDF5 structured complex.
    """
    arr = np.asarray(arr)
    # HDF5 structured complex: dtype has 'real' and 'imag' fields
    if arr.dtype.names and 'real' in arr.dtype.names and 'imag' in arr.dtype.names:
        arr = arr['real'] + 1j * arr['imag']
    # Native numpy complex
...
def load_and_average_fields(mat_path, entry_indices):
    ...
    for key in _AVERAGEABLE_KEYS:
        ...
        for f in fields:
            if key in f:
                arrays.append(_to_real_array(f[key]))
        if len(arrays) == n_pol:
...
def plot_single_fieldmap(data_2d, extent, x_label, y_label, title, cbar_label,
    ...
    # Display min/max values on colorbar
    cbar.ax.text(0.5, -0.03, f'{vmin:.2e}', transform=cbar.ax.transAxes,
                 ha='center', va='top', fontsize=9)
    cbar.ax.text(0.5, 1.03, f'{vmax:.2e}', transform=cbar.ax.transAxes,
...
def _generate_plots_for_field(field, args, n_unique_x, n_unique_y, extent, x_label, y_label,
    ...
    if spheres is not None and HAS_EDGE_FILTER:
        ...
        if detection_key is not None:
            det_data = _to_real_array(field[detection_key])
            if det_data.ndim == 1:
    ...
    for data_key, qty_label, cbar_label, cmap, fname_tag, int_mask_key in PLOT_CONFIGS:
        ...
        data_2d = _to_real_array(field[data_key])
