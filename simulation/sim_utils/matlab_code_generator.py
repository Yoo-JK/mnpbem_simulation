...
class MatlabCodeGenerator:
    ...
    def __init__(self, config, verbose=False):
        ...
        # Auto-disable iterative solver when substrate is used (known MNPBEM MEX bug)
        ...
        # segfaults regardless of serial/parallel execution.
        # Fallback: direct solver (bemretlayer) uses full matrices without MEX.
        use_iterative = config.get('use_iterative_solver', False)

        if use_substrate and use_iterative:
            print("[!] WARNING: Iterative solver is incompatible with substrate mode")
            print("    (MNPBEM MEX file 'hmatgreentab1' causes segmentation fault")
            print("     via bemretlayeriter -> aca.compgreenretlayer -> lowrank2)")
            print("    -> Automatically switching to direct solver (bemretlayer)")
            print("    [!] Note: Direct solver uses full N×N matrices (more memory)")
            self.config['use_iterative_solver'] = False
